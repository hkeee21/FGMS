...
template <int BLOCK_SIZE, int EX_LOOP, int IM_LOOP, 
  int SKEW, int M, int K, int N, int MS, int NS, int KS>
__global__ void _fgms_fusion_fp16_v3(
                const int *__restrict__ kpos,
                const int *__restrict__ qkpos, 
                const int k_vol, 
                const int c_in,
...
#pragma unroll
  for (int _n = 0; _n < c_out; _n += BLOCK_SIZE){
    // Loop over all the sub-matrices of A and B
    int cx_a = _n + ctx_a;
    int cx_b = _n + ctx_b;
#pragma unroll
    for (int _l = 0; _l < EX_LOOP; _l++){
      wmma::fill_fragment(c[_l], __float2half(0.0f));
    }
#pragma unroll
    for (int _k = 0; _k < c_in; _k += BLOCK_SIZE) {
      int b_load_x = ty / 4;
...
#pragma unroll
      for (int _l = 0; _l < EX_LOOP; _l++){
        ...
        int y_temp = y + _l * IM_LOOP * BLOCK_SIZE;
        int in_row = y_temp < __ldg(&kpos[widx + 1]) ? imap[y_temp] : -1;

        *((float4*)(&As[_l][ty][ctx_a])) = ((_k + ctx_a) < c_in && in_row > -1) ? 
          *((float4*)(&in_f[c_in * in_row + _k + ctx_a])) : 
          *((float4*)(&padding[0]));
      }
...
#pragma unroll
        for (int _l = 0; _l < EX_LOOP; _l++){
          wmma::load_matrix_sync(a[_l], &As[_l][warp_row * M][k], BLOCK_SIZE + SKEW);
          wmma::mma_sync(c[_l], a[_l], b, c[_l]); 
        }  
      }
      __syncthreads();
    }
...
#pragma unroll
    for (int _l = 0; _l < EX_LOOP; _l++){
      wmma::store_matrix_sync(&As[_l][warp_row * M][warp_col * N], 
        c[_l], BLOCK_SIZE + SKEW, wmma::mem_row_major);
    }
...
```for out channel = 64```
BLOCK_SIZE = 32,   
EX_LOOP = 2, IM_LOOP = 2, C_LOOP = 2
SKEW = 8, 
M = 16, K = 16, N = 16, 
MS = IM_LOOP * BLOCK_SIZE / M = 4, 
NS = C_LOOP * BLOCK_SIZE / N = 4, 
KS = warpNum / MS / NS = 1, 
blockDim.x = 4, blockDim.y = 64, blockDim.z = 2
*/
template <int BLOCK_SIZE, int EX_LOOP, int IM_LOOP, int C_LOOP, 
  int SKEW, int M, int K, int N, int MS, int NS, int KS>
__global__ void _fgms_fusion_fp16_v4(
                const int *__restrict__ kpos,
                const int *__restrict__ qkpos, 
                const int k_vol, 
                const int c_in,
  ...
  const int tid = threadIdx.z * blockDim.z 
    + threadIdx.y + blockDim.y + threadIdx.x;
  const int ctx_a = tx << 3;
  const int ctx_b = tx << 2;
  // const int tid = ty * blockDim.x + tx;

  // Warp index
  const int warpId = tid / 32;
  const int laneId = tid % 32;
  // const int warp_exl = warpId / (MS * NS); // 0, 1
  const int warp_row = warpId / NS; // 0, 1, 2, 3
  const int warp_col = warpId % NS; // 0, 1, 2, 3
  ...
  // Declaration of the shared memory array
  __shared__ half As[EX_LOOP][IM_LOOP * BLOCK_SIZE][BLOCK_SIZE + SKEW];
  __shared__ half Bs[BLOCK_SIZE][C_LOOP * BLOCK_SIZE + SKEW];
...
//   for (int _n = 0; _n < c_out; _n += BLOCK_SIZE){
    // Loop over all the sub-matrices of A and B
  int cx_a = threadIdx.z * BLOCK_SIZE + ctx_a;
  int cx_b = threadIdx.z * BLOCK_SIZE + ctx_b;
#pragma unroll
  for (int _l = 0; _l < EX_LOOP; _l++){
    wmma::fill_fragment(c[_l], __float2half(0.0f));
  }
#pragma unroll
  for (int _k = 0; _k < c_in; _k += BLOCK_SIZE) {
    int b_load_x = tid / (BLOCK_SIZE / 4);
    int b_load_y = tid % (BLOCK_SIZE / 4);
    // Kernel weight to Bs
    *((float2*)(&Bs[b_load_x][b_load_y])) = 
        ((_k + b_load_x) < c_in && b_load_y < c_out) ? 
        *((float2*)(kw_ptr + c_out * (b_load_x + _k) + b_load_y)) : 
        *((float2*)(&padding[0]));
...
#pragma unroll
    ...
    int y_temp = y + tz * IM_LOOP * BLOCK_SIZE;
    int in_row = y_temp < __ldg(&kpos[widx + 1]) ? imap[y_temp] : -1;

    *((float4*)(&As[tz][ty][ctx_a])) = ((_k + ctx_a) < c_in && in_row > -1) ? 
        *((float4*)(&in_f[c_in * in_row + _k + ctx_a])) : 
        *((float4*)(&padding[0]));
    // }
...
#pragma unroll
  ...
  const int warp_s = warpId / 8; // 0, 1
  const int warp_row_s = warpId % 8 / 2; // 0, 1, 2, 3
  const int warp_col_s = warpId % 2; // 0, 1
  // Store C fragments to shared memory
  // Note that we reuse As for Cs storing
#pragma unroll
  for (int _l = 0; _l < EX_LOOP; _l++){
    wmma::store_matrix_sync(&As[warp_s][warp_row_s * M][warp_col_s * N], 
      c[_l], BLOCK_SIZE + SKEW, wmma::mem_row_major);
